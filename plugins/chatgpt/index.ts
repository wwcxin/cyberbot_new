...
import fetch from 'node-fetch'; // 添加 fetch 导入，因为原生 fetch 可能不支持代理
...
const config = {
    ...
    RENDER: {
        markdownApi: 'https://md2html.b23.run',
        width: 900,
        initialHeight: 0,
        timeout: 30000
    },
...
async function mdToHtml(md: string): Promise<string> {
    try {
        const response = await fetch(config.RENDER.markdownApi, {
            method: 'POST',
            body: md,
            headers: {
                'Content-Type': 'text/plain',
            },
            agent: proxyAgent, // 使用 HTTP 代理
        });
